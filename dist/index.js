...
// Helper to get bots
async function fetchUserBots(telegramId) {
    try {
...
bot.command('start', async (ctx) => {
    ...
    await ctx.reply('Welcome to BotBuilder! 🤖\n\n' +
...
bot.callbackQuery(/^select_bot:(.+)$/, async (ctx) => {
    ...
    try {
        ...
        if (selectedBot) {
            ...
            await ctx.reply(`Now managing *${selectedBot.name}*.\n\nYou can ask me to:\n- "Show logs"\n- "Get stats"\n- "Deploy to prod"\n- "Improve the bot by adding a /help command"`, { parse_mode: 'Markdown' });
        }
        else {
...
bot.callbackQuery('confirm_action', async (ctx) => {
    if (!ctx.session.pendingAction || !ctx.session.activeBotId)
        return;
    const telegramId = String(ctx.from?.id);
...
bot.on('message:managed_bot_created', async (ctx) => {
    ...
    try {
        ...
        await ctx.reply(`🎉 Great! You've successfully created *${botName}*.\n\nNow, tell me what this bot should do. For example:\n_"A bot that sends a random joke every morning"_`, { reply_markup: { remove_keyboard: true }, parse_mode: 'Markdown' });
    }
    catch (error) {
